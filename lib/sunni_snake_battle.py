...
# Snake confuse move animation
elif current == "snake confuse move":
    
        if snake_confuse_x == 930:
            character_stage = idle_movement(character_stage,character,20,150,380)
            screen.blit(snake_normal, (930,440))
            snake_confuse_x -= 24
        elif snake_confuse_x > 450:
            character_stage = idle_movement(character_stage,character,20,150,380)
            if snake_position == "normal":
                screen.blit(snake_normal, (snake_confuse_x,440))
                snake_position == "moving"
            elif snake_position == "moving":
                screen.blit(snake_moving, (snake_confuse_x,440))
                snake_position == "normal"
            snake_confuse_x -= 24
                
        elif snake_confuse_x == 450:
            if duration_time < fps:
                if snake_confuse_direction == "backwards":
                    confuse_move_sound()
                    screen.blit(snake_backwards, (0,440))
                    screen.blit(character_normal, (150,380))
                    snake_confuse_direction = "forwards"
                elif snake_confuse_direction == "forwards":
                    screen.blit(snake_normal, (450,440))
                    screen.blit(character_backwards, (150,380))
                    snake_confuse_direction = "backwards"
                duration_time += 1
            else:
                screen.blit(snake_normal, (930,440))
                enemy_current_mana += 10
                if character_current_mana < 20:
                    confuse_damage = character_current_mana
                else:
                    confuse_damage = 20
                character_current_mana -= confuse_damage
                display_damage = font.render("-" + str(confuse_damage), True, MANA_BLUE)
                display_damage_time = 0
                snake_confuse_x = 0

        elif display_damage_time < fps/2:
            character_stage = idle_movement(character_stage,character,20,150,380)
            screen.blit(snake_normal, (930,440))
...
# Snake venom move animation
elif current == "snake venom move":
    screen.blit(snake_venom_stance, (930,440))
    character_stage = idle_movement(character_stage,character,20,150,380)
    ...
    if duration_time < 2*fps:
        if duration_time == 0:
            ...
            venom_damage = random.randint(15,25)
            if character_current_hp - venom_damage < 0:
                venom_damage = character_current_hp
            character_current_hp -= venom_damage                        
            display_damage = font.render("-" + str(venom_damage), True, DAMAGE_RED)
            display_damage_time = 0
            
        screen.blit(snake_venom_beam, (730,440))
        screen.blit(snake_venom_beam, (530,440))
        screen.blit(snake_venom_beam, (330,440))
        screen.blit(snake_venom_beam, (180,440))
        screen.blit(character_normal, (150,380))
        duration_time += 1
